...
export function timeFix() {
  ...
  return hour < 9 ? vvv.$t('common.home') : hour <= 11 ? '上午好' : hour <= 13 ? '中午好' : hour < 20 ? '下午好' : '晚上好'
}
...
export function generateIndexRouter(data) {
  window.mdata = data
  console.log('sfhsdhfhd')

  let indexUrl = getRootUrl(data)

  if (!indexUrl) {
    //抛出异常无权登录
    indexUrl = '/system/sysrolelist'
  }
  window.indexUrl = indexUrl
  let indexRouter = [
    {
      path: '/',
      name: 'dashboard',
...
function generateChildRouters(data) {
  ...
  for (let item of data) {
    if (item.menuType != 2) {
      ...
      if (item.perms && item.perms === 'noroute') {
        console.log(' 不生成路由 item.route：  ' + item.url)
        console.log(' 不生成路由 item.url  ' + item.url)
      } else {
        routers.push(menu)
      }
