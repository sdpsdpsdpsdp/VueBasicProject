    ...
    renderQueryItem() {
      return this.queryConfig.map(queryItem => {
        ...
        if (placeholder === undefined) {
          if (dictCode) {
            options.props['placeholder'] = vvv.$t('order.pleaseSelect') + label
          } else {
            options.props['placeholder'] = vvv.$t('order.pleaseInput') + label
          }
        } else {
        ...
        if (typeof customRender === 'function') {
          ...
          input =
            <j-dict-select-tag {...options} vModel={this.queryParam[key]} dictCode={dictCode} style='width:180px;' />
        } else {
