      ...
      <a-col :md='18' :sm='24'>
        <a-card :bordered='false'>
          ...
          <a-input-search
            ...
            :placeholder="$t('order.pleaseInputAccount')"
...
import { queryDepartTreeList, queryDeptTreeList, getUserList, queryUserByDepId } from '@/api/api'
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          customRender: (text) => {
            if (text === 1) {
              return this.$t('order.man')
            } else if (text === 2) {
              return this.$t('order.woman')
            } else {
      ...
      selectUserRows: [],
      selectUserIds: [],
      title: this.$t('order.selectUserFromDept'),
      ipagination: {
        current: 1,
        pageSize: 10,
  ...
  methods: {
    initUserNames() {
      if (this.userIds) {
        this.selectedRowKeys = this.userIds.split(',')
      }
    },
    async loadData(arg) {
      if (arg === 1) {
