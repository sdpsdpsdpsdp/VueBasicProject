<template>
  <global-layout @dynamicRouterShow="dynamicRouterShow">
    ...
    <a-tabs
      ...
      @edit="editPage">
      <a-tab-pane :id="page.fullPath" :key="page.fullPath" v-for="page in pageList" :closable="!(page.meta.title==$t('common.home'))">
...
<script>
  ...
  export default {
    ...
    created() {
      //20211014 如果用户没有首页权限 则登录后还会显示首页标签 点击报错 所以注释掉了
    ...
    methods: {
      ...
      changeTitle(title) {
        let projectTitle = this.$t('common.home')
        // 首页特殊处理
        if (this.$route.path === indexKey) {
      ...
      remove(key) {
        ...
        if (this.pageList.length === 1) {
          this.$message.warning(this.$t('common.lastCanNotClose'))
          return
        }
